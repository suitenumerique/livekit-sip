...
func (e *LivekitBin) cameraSleep(self *gst.Bin, p []lksdk.Participant) {
	...
	for _, part := range p {
		...
		if err := e.subscribeTrack(self, camera, "mosaic"); err != nil {
	...
	for _, part := range inactive {
		...
		if e.wantsTrack(camera.SID()) {
			e.markIdle(camera.SID(), func(self *gst.Bin) error { return e.releaseTrack(self, camera, "idle") })
		}
	}
}
